...
batchnum = round(config.train.total_imgs/config.train.batch_size_bot)
    ...
    with tf.device('/GPU:0'):
        with tf.Graph().as_default():
            with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
                ...
                '''if tf.is_nan(loss) is True: # This doesn't work...
                    sys.exit('Loss NaN, plz debug!')
                    #  to solve loss NaN: blog.csdn.net/lrt366/article/details/97616926'''
                ...
                # lh = sess.run(lh)
                # lc = sess.run(lc)
                '''plz modify fuc. trainlayer() & toatl_loss() so it returns specific loss term'''
                ...
                while True:
                    ...
                    '''tf.cond(tf.is_nan(loss_t),
                            true_fn=lambda: sys.exit('Loss NaN at epoch [%2d], global step: %4d' % (epoch + 1, step)),
                    ...
                    if batch_id == batchnum - 1:
                        if epoch >= goal_epoch:
                            ...
                            '''saving checkpoint'''
                            saver.save(sess, os.path.join(model_ckp, 'model-bot.ckpt'), global_step=step)
                        epoch += 1
...
def trainlayer(tfrecord_path, sess):
    ...
    '''reading tfrecord'''
    train_iter = train_iterator_bot(tfrecord_path)
    img_patch, gt_patch = train_iter.get_next()

    '''feeding network'''
    output = ns.bot_branch(img_patch)
    ...
    #  ssim = 1 - tf.reduce_mean(tf.image.ssim(output, gt_patch, max_val=1))
    #  msssim = 1 - tf.reduce_mean(tf.image.ssim(output, gt_patch, max_val=1)) branch
    # ---if running at tf1.4.0 at py27, use the self-made loss/ssim---
    lssim = 1 - ssim((output), (gt_patch), data_range=1)
    # msssim = 1 - ms_ssim(output, gt_patch, data_range=1)  # MS-SSIM not recommeded in bot

    '''unsupervised TMQI-SSIM loss (output vs. input, in a QA-like way)'''
    # ---TMQI-SSIM is NOT available even in higher version of tensorflow---
    ltmqi_s = 1 - ssim((output), (img_patch), data_range=1, tmqi=True)
    #  tmqi = 1 - ms_ssim(output, img_patch, data_range=1)  # MS-TMQI-SSIM not recommeded in bot

    '''hue shift loss (output vs. input, in a QA like way)'''
    lh = delta_hue_ipt((output), (img_patch), gamut1='709', gamut2='Fairchild Nikon D2x')

    '''color difference loss'''
    lc = delta_e_itp((output), (gt_patch), gamut1='709', gamut2='709')

    loss = l1 * 0.6 + lp * 0.3 + ltmqi_s * 0.15 + lh * 0.15 + lc * 0.2  # + loss_ssim * 0.2 + l2 * 0.1 + l2_reg * 0.1
    ...
    tf.summary.scalar('loss/loss_f', lp)
    tf.summary.scalar('loss/loss_ssim', lssim)
    # tf.summary.scalar('loss/loss_msssim', lmsssim * 0.2)
    tf.summary.scalar('loss/loss_tmqi_s', ltmqi_s)
    # tf.summary.scalar('loss/loss_msssim_QA', lmsssim_QA * 0.2)
